...
{
    internal static TradeLineItem Map(SPSTradeLineItemType source, MappingContext context) => new()
    {
        ...
        ScientificName = source.ScientificName?
            .Where(t => t.languageID == "la")
            .Select(t => t.Value)
            .ToList()
            .NullIfEmpty(),
        NetWeight = SpsMeasureMapper.Map(source.NetWeightMeasure),
        GrossWeight = SpsMeasureMapper.Map(source.GrossWeightMeasure),
        ApplicableProductClassification = SpsClassificationMapper.MapList(source.ApplicableSPSClassification, context),
