...
{
    internal static List<T>? NullIfEmpty<T>(this List<T>? list) =>
    ...
    internal static string? ForLanguage(this TextType[]? source, string languageCode) =>
        (source?.FirstOrDefault(t => t.languageID == languageCode)
         ?? source?.FirstOrDefault(t => t.languageID is null))?.Value;
    ...
    internal static List<string>? ForLanguageList(this TextType[]? source, string languageCode)
    {
        ...
        if (byLanguage is { Count: > 0 }) return byLanguage.Select(t => t.Value).ToList();
    ...
    /// <returns></returns>
    internal static string XmlEnumCode<T>(this T value) where T : struct, Enum
    {
        var memberName = value.ToString();
        if (typeof(T).GetField(memberName)
