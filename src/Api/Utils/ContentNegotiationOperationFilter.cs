...
internal sealed class ContentNegotiationOperationFilter : IOperationFilter
{
    ...
    {
        var multiContentResponses = context.ApiDescription.ActionDescriptor.EndpointMetadata
        ...
        {
            if (operation.Responses is null) break;
            if (!operation.Responses.TryGetValue(group.Key.ToString(), out var response) || response?.Content is null) continue;

            response.Content.Clear();
            foreach (var produces in group)
            {
                if (produces.Type is null) continue;
