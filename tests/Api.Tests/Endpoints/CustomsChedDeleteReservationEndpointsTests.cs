...
public class CustomsChedDeleteReservationEndpointsTests(TradeGatewayWebApplicationFactory factory)
{
    ...
    [Fact]
    public async Task Delete_ReleaseReservationSuccess()
    {
        StubSample(Ched, ReleasedSample);

        var response = await DeleteAsync(Ched, Mrn);

        response.StatusCode.Should().Be(HttpStatusCode.NoContent);
    }

    [Fact]
    public async Task Delete_ReleaseReservationFailed()
    {
    ...
    [Fact]
    public async Task Delete_ReleaseReservation_WhenUpstreamFault_ReturnsBadGatewayWithoutTheUpstreamMessage()
    {
        const string upstreamError = "internal upstream detail that must not be published";
        StubFault("FAULTY", upstreamError);

        var response = await DeleteAsync("FAULTY", "mrn");
    ...
    [Fact]
    public async Task Delete_ReleaseReservations_WhenSenderSaxFault_ReturnsInternalServerError()
    {
        StubSaxFault("BADSOAP");

        var response = await DeleteAsync("BADSOAP", "mrn");

        response.StatusCode.Should().Be(HttpStatusCode.InternalServerError);
    }

    private async Task<HttpResponseMessage> DeleteAsync(string chedId, string mrn)
    {
        var client = await factory.CreateClientForPrincipalAsync(Manager);
        return await client.DeleteChedReservation(chedId, mrn, TestContext.Current.CancellationToken);
    }
