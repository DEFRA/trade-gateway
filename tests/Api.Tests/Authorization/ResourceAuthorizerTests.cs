...
public class ResourceAuthorizerTests
{
    ...
    private static ResourceAuthorizer Build() => new(Options.Create(new AuthorizationConfig
    {
        ...
        Permissions = new Dictionary<string, List<PermissionGrant>>
        {
            ["intra-reader"] =
            [
                new PermissionGrant { Actions = ["READ", "WRITE"], Resource = "/certificates/intra/**" },
            ],
    ...
    public void Allows_read_on_granted_resource() =>
        Build().IsAuthorized(IntraSub, "/certificates/intra/ABC123", "GET").Should().BeTrue();
    ...
    public void Allows_write_when_granted() =>
        Build().IsAuthorized(IntraSub, "/certificates/intra/ABC123", "POST").Should().BeTrue();

    [Fact]
    public void Maps_put_to_write() =>
        Build().IsAuthorized(IntraSub, "/certificates/intra/ABC123", "PUT").Should().BeTrue();
    ...
    public void Denies_unmapped_methods(string method) =>
        Build().IsAuthorized(IntraSub, "/certificates/intra/ABC123", method).Should().BeFalse();

    [Fact]
    public void Denies_unknown_sub() =>
        Build().IsAuthorized("nobody", "/certificates/intra/ABC123", "GET").Should().BeFalse();

    [Fact]
    public void Denies_null_sub() =>
        Build().IsAuthorized(null, "/certificates/intra/ABC123", "GET").Should().BeFalse();
