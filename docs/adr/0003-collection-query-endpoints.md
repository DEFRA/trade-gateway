...
1. **Query parameters on the collection URL** — `GET /intras?updatedAfter=...&updatedBefore=...`
...
```
GET /intras?updatedAfter=2026-01-01T00:00:00Z&updatedBefore=2026-01-02T00:00:00Z&page=1&pageSize=20
GET /cheds?updatedAfter=...&updatedBefore=...&page=1&pageSize=20
GET /docoms?updatedAfter=...&updatedBefore=...&page=1&pageSize=20
```

This is the standard REST pattern for filtering a collection: query parameters narrow the result set without introducing a non-standard URL segment. The `updatedAfter`/`updatedBefore` names make the intent explicit without requiring a special operation segment.
...
|-----------|----------|-------------|
| `updatedAfter` | Yes | Return documents updated after this instant (exclusive). ISO 8601 UTC. |
| `updatedBefore` | Yes | Return documents updated before this instant (inclusive). ISO 8601 UTC. |
| `page` | No | 1-based page number. Defaults to 1. |
| `pageSize` | No | Items per page. Defaults to 20, capped at 200 (the upstream maximum). |

Both `updatedAfter` and `updatedBefore` are required on every request. Open-ended time windows are not supported — they would produce unbounded result sets.
...
| Code | When used |
|------|-----------|
| `400 Bad Request` | `updatedAfter` or `updatedBefore` is missing, unparseable, or `updatedAfter` ≥ `updatedBefore`. |
...
- `updatedAfter`/`updatedBefore` names are self-describing without a special URL segment.

**Negative / Trade-offs**

- `updatedAfter` / `updatedBefore` present on the collection URL blurs the distinction between "list all" and "poll for changes"; a consumer skimming the OpenAPI schema may not realise they are required parameters for the polling use case.
...
- Both `updatedAfter` and `updatedBefore` are required — consumers cannot request an open-ended "everything since" query.
